...
public class Script_Instance : GH_ScriptInstance
{
    ...
    public class ParticleSystem
    {
        ...
        public void UpdateJones()
        {
...

            {
                ...
                p.currentCPindex = ME.MeshPoints.ClosestPoint(p.pos);
            ...
            foreach (Particle p in Particles)
                ME.scalarField[p.currentCPindex] = (float)Math.Min(1.0, ME.scalarField[p.currentCPindex] + p.PheroStrength);
        ...
        public void UpdateRTree()
        {
            foreach (Particle p in Particles)
            {
                ...
                //// update scalar field
                int cpInd = ME.MeshPoints.ClosestPoint(p.pos);
                ME.scalarField[cpInd] = (float)Math.Min(1.0, ME.scalarField[cpInd] + p.PheroStrength);
        ...
        public DataTree<GH_Number> GetNeighBrightness()
        {
            ...
            {
                for (int j = 0; j < Particles[i].neighbours.Count; j++)
                    briOut.Add(new GH_Number(ME.scalarField[Particles[i].neighbours[j]]), new GH_Path(i));
            }
    ...
    public class Particle
    {
        ...
        public Particle(ParticleSystem pSystem, Point3d pos, Vector3d vel)
        {
            ...
            MaxSpeed = 1.5f;
            PheroStrength = 0.1f;
            visAng = pSystem.sensAng; //0.3 * Math.PI;
        ...
        public void SeekAngVis()
        {
            ...
            Vector3d rotAxis = pSystem.ME.MeshPoints[currentCPindex].Normal;
            ...
            pC = pSystem.ME.MeshPoints.ClosestPoint(pos + sensorC);
            pL = pSystem.ME.MeshPoints.ClosestPoint(pos + sensorL);
            pR = pSystem.ME.MeshPoints.ClosestPoint(pos + sensorR);
            briC = pSystem.ME.scalarField[pC];
            ...
            desired = pSystem.ME.MeshPoints[pSystem.ME.MeshPoints.ClosestPoint(pos + direction)].Location - pos;
            desired.Unitize();
            desired *= MaxSpeed;
        ...
        public void SeekColor()
        {
            ...
            if (neighbours.Count > 0)
            {
                ...
                {
                    bri = pSystem.ME.scalarField[neighbours[i]]; // brightness is 0-1 and RGB 0-255
                    if (bri > MaxBri)
                    {
                        MaxBri = bri;
                        MaxInd = i;
                ...
                desired += pSystem.ME.MeshPoints[neighbours[MaxInd]].Location - pos;
